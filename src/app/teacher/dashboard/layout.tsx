...
const NAV = [
  ...
  { href: "/teacher/dashboard/lessons", label: "授業予定", icon: (
    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
    </svg>
  )},
  { href: "/teacher/dashboard/reschedules", label: "振替リクエスト", icon: (
    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  { href: "/teacher/dashboard/notifications", label: "通知ログ", icon: (
    <svg className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
    ...
    <aside className="flex h-full flex-col bg-white">
      ...
      <nav className="flex-1 overflow-y-auto space-y-1 px-3 py-4">
        ...
        {NAV.map((item) => {
          ...
          const isReschedules = item.href === "/teacher/dashboard/reschedules";
          const badge = isDiagnosis ? pending : isMessages ? unread : isReschedules ? resched : 0;
          return (
