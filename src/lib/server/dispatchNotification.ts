...
export type NotifyActorKind = "parent" | "teacher" | "student";

export type NotifyPayload = {
  actor_kind: NotifyActorKind;
  actor_id: string;
  event_type: string; // new_parent_message / reschedule_decision / announcement / ...
  subject: string;
  body_text: string;
  body_html?: string;
  link?: string;
...
async function lookupContactEmail(supabase: any, actor_kind: NotifyActorKind, actor_id: string, override: string | null): Promise<string | null> {
...
/**
 * メール / Web Push / LINE へ通知を送る。受信者の notification_preferences に従う。
 * 認可は呼び出し側で済ませる前提（service role で動作）。
 */
